...
import { STATIONS, TYPES, QUARTIERS, CENTRE } from "./carteDonnees.js";

/**
 * Niveau 3 — cumulatif : tout le niveau 2 (filtres par niveau, recherche, quartier,
 * valeur) plus des symboles proportionnels PARAMÉTRABLES. L'utilisateur choisit ce
 * que la TAILLE encode (pondération : valeur, fraîcheur, observations) et en combien
 * de PALIERS la découper. Synthétiser et comparer : les grandeurs deviennent visibles.
 */
const CRITERES = {
...
export default function CarteParametrable() {
  ...
  // pondération : échelle globale (toutes les stations) pour des paliers stables
  ...
  return (
    <div className="cv">
      ...
      <MapContainer ref={setCarte} center={CENTRE} zoom={13} scrollWheelZoom={false} className="cv-map">
        ...
        <CoucheMarqueurs lib={lib} stations={visibles} tailleDe={tailleDe} />
      </MapContainer>

      <p className="cv-compte">
        {visibles.length} stations · taille des icônes = {CRITERES[critere].libelle.toLowerCase()} en {paliers} paliers (petit = faible, grand = élevé)
      </p>
