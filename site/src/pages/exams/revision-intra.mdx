...
<Checkpoint question="Pourquoi DELETE /boites/12 plutôt que GET /boites/supprimer/12 ?">
...
- Chaque ressource s'expose comme une **collection** (`/boites`) et des **éléments** (`/boites/:id`), manipulés par les verbes standards.
- Une **relation** se traduit, lorsque c'est naturel, par un chemin imbriqué (`/boites/:id/emprunts`), sans empiler les niveaux à l'excès.
- Chaque opération a un **code de statut** attendu, en succès comme en erreur (création `201`, ressource absente `404`, données invalides `400`).
- Une bonne API est **prévisible et cohérente** : mêmes conventions de nommage, de pluriel et de format d'un endpoint à l'autre.
- Concevoir, c'est **raconter une histoire** lisible : à la seule lecture des endpoints, on devine le domaine et les usages.

<Checkpoint question="Par quoi commence la conception d'une API, avant d'écrire le moindre endpoint ?">
...
<Checkpoint question="L'API Lino doit lister les boîtes d'un quartier, consulter une boîte précise, et consulter les emprunts d'une boîte. Proposez les endpoints.">
...
<Checkpoint question="Pour exposer un historique d'emprunts potentiellement très long, comment concevoir l'endpoint afin qu'il reste utilisable ?">
...
<Checkpoint question="Concevez le flux d'échanges pour ce scénario : Alice repère une boîte de son quartier, consulte les livres qui y sont disponibles, puis en emprunte un.">
Une séquence d'échanges requête-réponse, du repérage vers l'écriture :
1. `GET /boites?quartier=Rosemont` répond `200` avec les boîtes du quartier.
2. `GET /boites/12/livres?disponible=true` répond `200` avec les livres disponibles de la boîte choisie.
3. `POST /boites/12/emprunts` (corps : `{ "livre_id": 87 }`) répond `201 Created` avec l'emprunt enregistré.
...
- Pour supporter un accès à la BD **asynchrone**, le gestionnaire emploie `async`/`await` et traite explicitement l'absence de résultat ainsi que les erreurs.
